...
namespace AcessoWebApi.Controllers
{
  ...
  { 
      ...
      { 
        ...
        [Authorize(Policy = "User")]
        [HttpPost]
        public async Task<ActionResult> Post([FromBody] LojaDTOCreate loja) 
        {
            if (!ModelState.IsValid)  
            { 
                return BadRequest(ModelState); 
           ...
           try 
           { 
                var result = await _service.Post(loja); 
                if (result != null) 
                { 
                    return Ok(result); 
                } 
        ...
        [Authorize(Policy = "User")]
        [HttpPut]
        public async Task<ActionResult> Put([FromBody] LojaDTOUpdate loja) 
        { 
           if (!ModelState.IsValid) 
           { 
                return BadRequest(ModelState); 
           ...
           try  
           {   
                var result = await _service.Put(loja); 
                if (result != null) 
                { 
                    return Ok(await _service.Get(result.Idloja)); 
                } 
                else 
                { 
                    return BadRequest(); 
